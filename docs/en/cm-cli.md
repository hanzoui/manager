# `cm-cli`: Hanzo Manager CLI

`cm-cli` is a tool that allows you to use various functions of Hanzo Manager from the command line without launching Hanzo Studio.
...
* For instance, executing the command `python cm-cli.py update all --channel recent --mode remote` will operate based on the latest information from remote rather than local data embedded in the current Hanzo Manager repo and will only target the list in the recent channel.
...
`[installed|enabled|not-installed|disabled|all|snapshot|snapshot-list]`
   ...
   * `snapshot-list`: Shows a list of snapshot files stored in Hanzo Manager/snapshots.
...
* `python cm-cli.py restore-snapshot <snapshot .json/.yaml>`: Restores to the specified snapshot.
  ...
  * If no file exists at the snapshot path, it is implicitly assumed to be in Hanzo Manager/snapshots.
...
You can set whether to use Hanzo Manager solely via CLI.

`cli-only-mode [enable|disable]`

* This mode can be used if you want to restrict the use of Hanzo Manager through the GUI for security or policy reasons.
    * When CLI only mode is enabled, Hanzo Manager is loaded in a very restricted state, the internal web API is disabled, and the Manager button is not displayed in the main menu.
...
In the GUI, installations, updates, or snapshot restorations are scheduled to execute the next time Hanzo Studio is launched. The `clear` command clears this scheduled state, ensuring no pre-execution actions are applied.
